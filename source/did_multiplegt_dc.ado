...
capture program drop did_multiplegt_dc
program did_multiplegt_dc, eclass
	version 12.0
	syntax varlist(min=4 numeric) [if] [in]  [, RECAT_treatment(varlist numeric) THRESHOLD_stable_treatment(real 0) trends_nonparam(varlist numeric) trends_lin(varlist numeric) controls(varlist numeric) weight(varlist numeric) placebo(integer 0) dynamic(integer 0) breps(integer 0) cluster(varlist numeric) covariances average_effect(string) save_results(string) text(passthru)]
...
gen treatment_effect_upper_90CI=.
gen treatment_effect_lower_90CI=.
...
replace treatment_effect_upper_95CI=placebo_`i'_2+invnormal(0.975)*se_placebo_`i'_2 if _n==`placebo'-`i'+1
replace treatment_effect_lower_95CI=placebo_`i'_2-invnormal(0.975)*se_placebo_`i'_2 if _n==`placebo'-`i'+1

replace treatment_effect_upper_90CI=placebo_`i'_2+invnormal(0.95)*se_placebo_`i'_2 if _n==`placebo'-`i'+1
replace treatment_effect_lower_90CI=placebo_`i'_2-invnormal(0.95)*se_placebo_`i'_2 if _n==`placebo'-`i'+1
}
}
...
replace treatment_effect_upper_95CI=effect_0_2+invnormal(0.975)*se_effect_0_2 if _n==`placebo'+1 
replace treatment_effect_lower_95CI=effect_0_2-invnormal(0.975)*se_effect_0_2 if _n==`placebo'+1 
replace treatment_effect_upper_90CI=effect_0_2+invnormal(0.95)*se_effect_0_2 if _n==`placebo'+1 
replace treatment_effect_lower_90CI=effect_0_2-invnormal(0.95)*se_effect_0_2 if _n==`placebo'+1 
...
replace treatment_effect_upper_95CI=effect_`i'_2+invnormal(0.975)*se_effect_`i'_2 if _n==`placebo'+`i'+1 
replace treatment_effect_lower_95CI=effect_`i'_2-invnormal(0.975)*se_effect_`i'_2 if _n==`placebo'+`i'+1 
replace treatment_effect_upper_90CI=effect_`i'_2+invnormal(0.95)*se_effect_`i'_2 if _n==`placebo'+`i'+1 
replace treatment_effect_lower_90CI=effect_`i'_2-invnormal(0.95)*se_effect_`i'_2 if _n==`placebo'+`i'+1 
}
}

twoway (line treatment_effect time_to_treatment, lpattern(solid)) (rcap treatment_effect_upper_90CI treatment_effect_lower_90CI time_to_treatment, lwidth(1.15) lcolor(gs7)) (rcap treatment_effect_upper_95CI treatment_effect_lower_95CI time_to_treatment, lcolor(gs12)), xlabel(-`placebo'[1]`dynamic') xtitle("Time since treatment", size(large)) ytitle("Treatment effect", size(large)) graphregion(color(white)) plotregion(color(white)) legend(off) `text' yline(0, lcolor(red))
